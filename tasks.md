...
| T056 | data | T010 | todo | Закрыть гонку со станцией: зафиксировать привязку станции в момент публикации версии, а не читать её из мутируемой checklists.stationId при сохранении заполнения. Тест на гонку: перевязать чек-лист на другую станцию между выдачей версии и сохранением — заполнение обязано остаться на станции, где его заполняли | — | 1 |
| T057 | data | T009 | todo | Ограничение в базе на равные границы окна времени: чек-лист с window_start = window_end никогда не отдаётся ни на одной станции и делает это молча. Тест: вставка с равными границами отвергается базой | — | 1 |
| T058 | data | T009 | todo | Ограничения размера JSONB в базе для публично записываемых данных: верхняя граница на pg_column_size(answers) и pg_column_size(snapshot) в submissions и на sections в checklist_versions. Тест: заведомо огромный JSON отвергается базой, а не только кодом | — | 1 |
| T059 | data | T012 | todo | Устойчивый порядок ленты: вторичный ключ сортировки (submitted_at, id) и составной индекс (station_id, submitted_at desc) под фильтр ленты по станции и периоду. Тест: 500 заполнений с одинаковой отметкой времени отдаются в одном и том же порядке при повторных запросах | — | 1 |
| T060 | data | T008 | todo | Таймауты пула соединений: statement_timeout и connectionTimeoutMillis, явный max под публичную точку записи. Без них всплеск нагрузки копит зависшие запросы вместо быстрого отказа | — | 1 |
| T061 | data | T011 | todo | Тест на пересечение окон двух чек-листов одной станции: сейчас правило «берётся начинающийся раньше» живёт только комментарием в коде и ни разу не запускалось | — | 1 |
