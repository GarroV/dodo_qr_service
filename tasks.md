...
| T064 | core | T001 | in_progress | Подключить Golos Text (эталон грузит его из Google Fonts, в приложении шрифта нет вовсе) и базовый размер текста 13px/18px по эталону: сейчас браузерные 16px, и каждый следующий экран будет расходиться с эталоном | — | 1 |
| T065 | core | T001 | in_progress | Заголовки безопасности в next.config.ts: Content-Security-Policy и X-Frame-Options. Сейчас их нет ни на одной странице, включая публичный маршрут заполнения | — | 1 |
...
| T069 | core | T004 | in_progress | playwright.config.ts жёстко занимает порт 3101 при reuseExistingServer: true. Если приёмка и чей-то прогон стартуют одновременно, сквозные сценарии молча проверят чужую сборку и покажут зелёный результат по чужому коду | — | 1 |
| T070 | core | T008 | in_progress | Тестовая база одна на все копии репозитория: два одновременных прогона сносят друг другу схему, и упавший тест выглядит случайным. Развести адрес тестовой базы по копиям — как разведены порты и имена compose-проектов | — | 1 |
