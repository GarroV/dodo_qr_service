...
| T069 | core | T004 | todo | playwright.config.ts жёстко занимает порт 3101 при reuseExistingServer: true. Если приёмка и чей-то прогон стартуют одновременно, сквозные сценарии молча проверят чужую сборку и покажут зелёный результат по чужому коду | — | 1 |
