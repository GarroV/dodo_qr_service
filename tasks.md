...
| T063 | auth | T014 | in_progress | Сквозные сценарии не должны зависеть от .env: сейчас файл, скопированный из .env.example по инструкции проекта, перекрывает переменные, которые playwright.config.ts передаёт тестовому серверу, и 6 сценариев входа падают. Без .env те же 23 сценария зелёные. Починить так, чтобы прогон из свежего клона проходил и с .env, и без него | — | 1 |
...
| T066 | auth | T063 | in_progress | Ограничение частоты попыток входа: пароль один на всю сеть и админка смотрит в интернет, перебор ничем не ограничен | — | 1 |
