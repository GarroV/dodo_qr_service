...
describe("проверка окружения при старте", () => {
  ...
  it("предупреждает, что базовый путь доведён не до конца", () => {
    // Ссылки продукта (обычные <a href>) базового пути не знают: на пути работают
    // адреса страниц и напечатанный QR, а переходы между экранами админки уводят
    // на корень. Пока это так, включённый BASE_PATH обязан говорить о себе вслух.
