...
/**
 * Разделитель частей хэша — точка, а НЕ `$` из формата PHC.
 *
 * Хэш живёт в переменной окружения, а окружение приложения собирает загрузчик `.env`
 * (`@next/env`), который прогоняет значения через подстановку `$переменная` — и не только
 * строки из файла, но и те, что уже стоят в окружении процесса, если файл упоминает то же
 * имя. `scrypt$32768$8$3$соль$ключ` превращается в `scrypt-ключ` молча, без единой строки
 * в логе, и вход отказывает «испорченным хэшем» у того, кто всё сделал по инструкции.
 * В base64url точки нет, поэтому разбор однозначен, а подстановка такой строки не касается.
 * Проверка на это: `env-file.test.ts`.
 */
const SEPARATOR = ".";
const HASH_PARTS = 6;
const KEY_LENGTH = 32;
