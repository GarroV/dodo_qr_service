...
/**
 * Кто прислал запрос — по заголовкам обратного прокси. `null` означает «различить
 * клиентов нечем», и это НЕ повод считать всех одним ключом: предел «на клиента»,
 * применённый к одному общему ключу, перестаёт быть пределом на клиента и становится
 * рубильником, который гасит всю сеть, когда на пересменке сканируют одновременно.
 * Публичная площадка продукта — туннель, который может не передавать адрес клиента
 * (D034), поэтому случай реальный, а не теоретический. Точку записи в базу при этом
 * прикрывают пределы отправок: они считаются по коду станции и по сети целиком и от
 * адреса не зависят вовсе.
 */
